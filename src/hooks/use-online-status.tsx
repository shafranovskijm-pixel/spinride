...
async function processOrderSync(item: { action: string; data: unknown }) {
  if (item.action === 'create') {
    ...
    // Use server-side validated RPC: prices and totals are recalculated from DB
    const cartItems = (orderData.items || [])
      .filter((i) => i && i.product_id && i.quantity)
      .map((i) => ({ product_id: i.product_id, quantity: i.quantity }));

    const { error } = await supabase.rpc('create_validated_order', {
      _customer_name: orderData.customer_name,
      _customer_phone: orderData.customer_phone,
      _customer_email: orderData.customer_email ?? null,
      _delivery_method: orderData.delivery_method,
      _delivery_address: orderData.delivery_address ?? null,
      _notes: orderData.notes ?? null,
      _cart_items: cartItems as unknown as import('@/integrations/supabase/types').Json,
    });
